<template>
  ...
  <div v-else>
    <div
      v-if="data.length > 0"
      v-for="applied in data"
      class="grid grid-cols-10 gap-4 mb-4">
      <img
        :src="applied.job.company.image ?? DefaultImg"
        alt=""
        class="col-span-1" />

      <div class="col-span-8">
        <RouterLink
          :to="`/jobs/${applied.job.$id}`"
          class="font-semibold hover:underline"
          >{{ applied.job.title }}</RouterLink
        >
      ...
      <RouterLink
        :to="{
          name: 'chat',
          query: {
            receivedId: applied.job.company.$id,
